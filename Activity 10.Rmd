...
# This code chunk was included to increase image quality, as well as made code chunks not visible to the reader until the code appendix. It also was included to import the groundhog package, which aids in the importation of other packages in a future code chunk.
knitr::opts_chunk$set(echo = FALSE, dpi = 300, warning = FALSE)
library(groundhog)
```

```{r loadPackages}
# This code chunk was included to load in the packages needed for future code chunks on this R Markdown file, using the groundhog package. 
#Load packages with groundhog to improve stability
library("groundhog")
pkgs <- c("ggplot2", "tidyverse", "tinytex", "janitor", "knitr", "kableExtra", "dplyr")
...
# This code chunk was included to create the get_collatz_conjecture function, which takes the input of a number and outputs the step. The step is determined by how many times the get_collatz_conjecture has to run or recurse until the input number reaches 1. 
...
# This code chunk was included to vectorize the collatz conjecture function to convert the function from scalar to vector, which allows for the creation of a new object, called the collatz_conjecture, to cycle the first 10,000 numbers (integers) through the get_collatz_conjecture function. The get_collatz_conjecture function returns how many times the function had to recurse before reaching the number 1. 
...
# This code chunk was included to transform the collatz_data object into a data frame that could be used to create a histogram for the first 10,000 numbers in the Collatz Conjecture with their stopping times. The stopping times come from how many times the Collatz Conjecture has to run before the number hits 1. 
...
# The code chunk was included to create a scatter plot of the diamonds data by showing the relationship between diamond carat, price, and color.
ggplot(diamonds) +
...
```{r diamondsDataVisualizationCutPrice}
# The code chunk was included to create a jitter plot of the diamonds data by showing the relationship between diamond cut, price, and color.
ggplot(diamonds) +
...
# The code chunk was included to create a summary table for the diamonds data grouped by cut. The summary table displays a 7 number summary, the count, and the arithmetic mean and arithmetic standard deviation of the diamonds data grouped by cut.
# In addition, the code chunk was included to provide a more visually appealing summary table through the use of the kableExtra package.
